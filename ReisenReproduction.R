...
noaa = read_csv("NOAA weather data 1994 1995 Mecca FS.csv",n_max=727)
noaa = noaa %>% select(DATE,TMAX,TOBS)
noaa = noaa %>% mutate(T = date_to_T(DATE))
noaaavg = noaa %>%
          select(T,TMAX,TOBS) %>%
		  group_by(T) %>%
...
plot(gmod_red, which=4, cook.levels=cutoff)
influencePlot(lmC,
