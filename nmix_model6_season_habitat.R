...
model.open.covariate = pcountOpen(~SLTMRSH+DKPND+RCRP+GRP+CIT+DAT+PST+FSH+TOTAL,
								  ~SLTMRSH+DKPND+RCRP+GRP+CIT+DAT+PST+FSH+TOTAL,
								  ~SLTMRSH+DKPND+RCRP+GRP+CIT+DAT+PST+FSH+TOTAL,
								  ~season,
								  y,
                        		  mixture="P",
                        		  ...
                        		  dynamics="constant",
                          		  fix="none",
                        		  starts=c(model.closed@opt$par[1],rep(0,30),rep(model.closed@opt$par[2],2)),
                        		  method=Method,
                        		  se=TRUE,
                        		  immigration=FALSE,
                        		  iotaformula=~1)
...
saveRDS(model.open.covariate,file="nmix_model6_season_habitat.rds")
